...
let
  ...
  hevmCompliance = x: x.runCommand "hevm-compliance" {} ''
    ...
    ${x.pkgs.hevm}/bin/hevm compliance \
      ...
      --skip "(RevertPrecompiledTouch_storage|Create2Recursive|Create1000|recursiveCreateReturn|randomStatetest647|multiOwnedRemoveOwner|walletRemoveOwnerRemovePending|static_Call50000)" \
      --timeout 20 \
      --html > $out/index.html
